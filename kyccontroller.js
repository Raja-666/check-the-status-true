...
const handleGetKyc = async (req, res) => {
  ...
  const handleUpdateKyc = async (req, res) => {
    try {
      ...
      const { frontStatus, backStatus, selfiStatus } = await Kyc.findOne({ _id: reqData._id });
      if (frontStatus && backStatus && selfiStatus) {
        // If all three images are approved, send the approval mail to the user
        const mailOptions = {
          from: 'rajahulk@gmail.com',
          to: 'rajahulk@gmail.com', // Update the recipient email address
